  ...
  if res.status_code == 200:
    ...
    for i in ints:
        ...
        res, continuation_token = reviews(
            PLAYSTORE_ID, count=BATCH_SIZE, continuation_token=continuation_token)
        result.extend(res)
        t.update(BATCH_SIZE)
    t.close()
    ...
    run_date = datetime.now() + timedelta(hours=delta_timeunit)
    scheduler.add_job(func=delete_table_job, trigger="date",
                      run_date=run_date, args=[google_sqlite_table])
...
def final_by_date(google_neg_rev, start_date, end_date):
  mask = (google_neg_rev['at'] > start_date) & (
      google_neg_rev['at'] <= end_date)
