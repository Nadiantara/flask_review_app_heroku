...
from flask_test.webapp_functions.processing_tools import get_negreview_topic, get_importancescore, get_urgencyscore, get_priority_score_scaled, sql_get_preprocessed, get_negreview_topic2
    ...
    if(app_id is not None and country_code is not None):
        ...
        if(conn.dialect.has_table(conn.connect(), temp_name)):
            try:
                ...
                sql_get_preprocessed(app_id, country_code, conn)
                neg_dataframe, topic = get_negreview_topic2(
                    app_id, country_code, conn)
                priority_score = pd.merge(get_importancescore(app_id, country_code, conn, neg_dataframe, topic), get_urgencyscore(
                    fetched_df, neg_dataframe, topic), on='Topic')
                priority_score_scaled = get_priority_score_scaled(
                    priority_score)
...
def fetch_sentiment_plots():
    ...
    return (make_sentiment_plots(app_id, country_code, start_date, end_date))
