...
from flask_test.webapp_functions.processing_tools import get_negreview_topic, get_importancescore, get_urgencyscore, get_negreview_topic2
...
def plot_alt_totalreview_1(dataframe, start_date, end_date):
  ...
  base = alt.Chart(df[-12:].reset_index()).encode(
      alt.X('at:T', timeUnit='yearmonth', axis=alt.Axis(labels=True,
                                                        title=None, grid=False, tickCount=12, labelColor='black')),
...
def OLD_plot_alt_totalreview_2(dataframe):
    ...
    grouped_multiple = df.groupby(
        ['version', 'label']).agg({'review': ['count']})
...
def plot_alt_totalreview_2(dataframe, start_date, end_date):
    ...
    grouped_multiple = grouped_multiple.loc[grouped_multiple['version'].isin(
        grouped_multiple['version'].unique()[-13:])]
    ...
    # Plot
    base = alt.Chart(grouped_multiple).encode(
        alt.X('version:O', axis=alt.Axis(title=None, ticks=False,
                                         grid=False, titleFontSize=20, labelColor='black', labels=True)),
        tooltip=['version', 'label:N', 'review']
    ).properties(
        width=alt.Step(50),
        height=300
...
def plot_alt_rating_3(dataframe):
    ...
    grouped_multiple.sort_values(
        by=['rating', 'review'], inplace=True, ascending=False)
    grouped_multiple['value'] = (
        grouped_multiple['review'] / grouped_multiple['review'].sum()) * 100
    grouped_multiple['value'] = grouped_multiple['value'].round(
        2).astype(str) + '%'
    ...
    base = alt.Chart(grouped_multiple).transform_joinaggregate(
        ...
        alt.X('PercentofReview:Q', axis=alt.Axis(title=None, grid=False, titleFontSize=20,
                                                 labelColor='black', tickCount=0, format=('.0%'), labels=False)),
...
def plot_alt_total_negative_three(DATAFRAME, NEG_DATAFRAME, TOPIC, start_date, end_date):
  ...
  for i in range(len(df_neg)):
    for kolom in topic:
      if df_neg[kolom][i] > 0:
        df_neg.at[i, kolom] = round(
            (df_neg[kolom][i] / df_neg['review'][i] * 100), 2)
  ...
  new_data = no_review.melt(
      id_vars=["at"], var_name="Topic", value_name="Total")
  ...
  selection = alt.selection_single(
      fields=['Topic'], bind=input_dropdown, name='Topic')
...
def plot_alt_total_negative_trendline(DATAFRAME, NEG_DATAFRAME, TOPIC, start_date, end_date):
  ...
  input_dropdown = alt.binding_select(options=topic+[None])
  selection = alt.selection_single(
      fields=['Topic'], bind=input_dropdown, name='Selected')
  color = alt.condition(selection,
                        alt.Color('Topic:N', legend=None),
                        alt.value('lightgray'))
...
def plot_alt_prioritymatrix(APPID, COUNTRY, dataframe, neg_dataframe, topic, x=6):
    '''
    ...
    conn = db.engine
    priority_score = pd.merge(get_importancescore(APPID, COUNTRY, conn, neg_dataframe,
                                                  topic, x), get_urgencyscore(dataframe, neg_dataframe, topic, x), on='Topic')
    ...
    chart = alt.Chart(priority_score).mark_circle(size=250).encode(
        alt.X('importance_score_scaled', axis=alt.Axis(
            title='Importance', values=[0, 0.5, 1])),
        alt.Y('urgency_score_scaled', axis=alt.Axis(
            title='Urgency', values=[0, 0.5, 1])),
...
def make_sentiment_plots(APPID, COUNTRY, start_date, end_date):
  #connect to SQLite database
  temp_name = f"{APPID}_{COUNTRY}"
  start_date = start_date
  end_date = end_date
  conn = db.engine
  # fetch dataset
  fetched_df = pd.read_sql_table(temp_name, conn)
  neg_df, topic = get_negreview_topic2(APPID, COUNTRY, conn)
  ...
  return json.dumps([
      ...
      plot_alt_total_negative_trendline(
          fetched_df, neg_df, topic, start_date, end_date).to_json(),
      plot_alt_prioritymatrix(
          APPID, COUNTRY, fetched_df, neg_df, topic).to_json()
  ])
