...
def sql_get_preprocessed(APPID, COUNTRY, db_connection):
    table_name = f'{APPID}_{COUNTRY}'
    sql_preprocessed_table_name = f'PREPROCESSED_{APPID}_{COUNTRY}'
    if(db_connection.dialect.has_table(db_connection.connect(), sql_preprocessed_table_name)):
      #df = pd.read_sql_table(sql_preprocessed_table_name, db_connection)
      return True
    else:
      try:
        start = time.perf_counter()
        print(">>>>>>>>>>>>>get_preprocess_data CALLED <<<<<<<<<<<")
        df1 = pd.read_sql_table(table_name, db_connection)
        #df = parallelize_dataframe(df1, get_preprocess_data)
        df = get_preprocess_data(df1)
        df.to_sql(sql_preprocessed_table_name,
...
def get_trigrams(pos_bigram_reviews, neg_bigram_reviews):
  ...
  pos_trigrams = gensim.models.Phrases(
      pos_bigram_reviews, min_count=2, threshold=15)
  pos_trigram_reviews = pos_trigrams[pos_bigram_reviews]
  neg_trigrams = gensim.models.Phrases(
      neg_bigram_reviews, min_count=2, threshold=8)
...
def get_n_grams(df):
  ...
  pos_trigram_reviews, neg_trigram_reviews = get_trigrams(
      pos_bigram_reviews, neg_bigram_reviews)
...
def get_negreview_topic2(APPID, COUNTRY, db_connection):
    '''
    ...
    # vec is the count vectorizer associated with the LDA model
    vec = pickle.load(open(f'{MODEL_BASE_DIR}dummy_cv.pkl', mode='rb'))
    # LDA is the pretrained lda model
    LDA = pickle.load(open(f'{MODEL_BASE_DIR}dummy_lda.pkl', mode='rb'))

    # make sure to take the negative set here
    dataframe = pd.read_sql_table(
        f'PREPROCESSED_{APPID}_{COUNTRY}', db_connection)
    neg_df_1 = dataframe[dataframe['rating'] < 4]
    negative_preprocessed = neg_df_1
    negative_preprocessed_value = negative_preprocessed['review'].values
    topic_names = ['network_issues', 'topic_2', 'interface/listing', 'scammers/transaction_experience',
                   'topic_4', 'updates/message/notif_issues']  # transaction related--> everything between buyer seller
    ...
    topic_distributed_df_raw = pd.concat(
        [negative_preprocessed[['reviewId']].reset_index(drop=True), raw_distributions], axis=1)

    topic_distributed_df = pd.concat(
        [negative_preprocessed[['reviewId']].reset_index(drop=True), vectorized_df], axis=1)
...
def get_negreview_topic(dataframe):
    ...
    # vec is the count vectorizer associated with the LDA model
    vec = pickle.load(open(f'{MODEL_BASE_DIR}dummy_cv.pickel', mode='rb'))
    # LDA is the pretrained lda model
    LDA = pickle.load(open(f'{MODEL_BASE_DIR}dummy_lda.pickel', mode='rb'))
...
def get_init_params(df):
  ...
  df_train, df_test = train_test_split(
      df, test_size=0.05, stratify=df.rating.values, random_state=2020)
...
def get_train_model(x_train, y_train):
  ...
  log_r = LogisticRegression(
      random_state=2020, class_weight='balanced', solver='liblinear')
...
def get_coeff_df(vocab_size, features, coeffs):
  result_dict = dict(tuple([(features[i], coeffs[0][i])
                            for i in range(vocab_size)]))
  result_df = pd.DataFrame(
      sorted(result_dict.items(), key=lambda x: x[1]), columns=['word', 'coeff'])
...
def get_outlier_scaling(df, upper, lower):
  ...
  if lower > 0:
    ...
    df['score'].loc[small] = df['score'].loc[small] * \
        abs(df['z_score'].loc[small])
  return df
...
def get_match_merge_df(combined_df, scored_scaled_df, similar_index, neg_dataframe):
  ...
  final_df = pd.merge(combined_df.loc[similar_index][[
                      'review', 'score']], neg_dataframe, how='inner', on=['review'])  # merge
...
def get_severity_scores2(APPID, COUNTRY, db_connection, neg_dataframe, topic):
  dataframe = pd.read_sql_table(
      f'PREPROCESSED_{APPID}_{COUNTRY}', db_connection)
  review_rating_dict = {
  ...
  similar_index = combined_df[combined_df['review'].isin(
      neg_dataframe['review'])].index
  combined_df_preprocessed = combined_df
  ...
  lower_outlier, upper_outlier = get_find_outliers(
      scored_unscaled_df.lengths.values)
  scored_scaled_df = get_outlier_scaling(
      scored_unscaled_df, upper_outlier, lower_outlier)
  final_df = get_match_merge_df(
      combined_df, scored_scaled_df, similar_index, neg_dataframe)
  ...
  raw_sum_dict = {k: v for k, v in sorted(
      raw_sum_dict.items(), key=lambda item: item[1])}  # sort this
  normalized_sum_dict = {k: v for k, v in sorted(
      normalized_sum_dict.items(), key=lambda item: item[1])}
...
def get_importancescore(APPID, COUNTRY, db_connection, neg_dataframe, topic, x=6):
    '''
    ...
    dataframe = pd.read_sql_table(
        f'PREPROCESSED_{APPID}_{COUNTRY}', db_connection)
    severity_score = get_severity_scores2(
        APPID, COUNTRY, db_connection, neg_dataframe, topic)
    df = neg_dataframe
    topic = topic
    df_cubi = dataframe
    df_cubi.index = df_cubi['at']
    df.index = df['at']
    df = df[topic].resample('MS').sum().join(
        pd.DataFrame(df_cubi['review'].resample('MS').count()))
    for kolom in topic:
    ...
    for i in range(len(topic)):
      x1 = statistics.mean(df[topic[i]][-x:]) / 100 * \
          sum(df['review'][-x:]) * severity_score[topic[i]][0]
      score.insert(i, x1)
...
def get_urgencyscore(dataframe, neg_dataframe, topic, x=6):
    ...
    score = []
    for i in range(len(topic)):
      y = -(statistics.mean(df[topic[i]][-x:]) -
            df[topic[i]][-1:]).sum() / np.std(df[topic[i]])
      score.insert(i, y)
...
def get_priority_score_scaled(priority_score):
  ...
  for i in range(len(priority_score)):
    z = (priority_score['importance_score'][i] - priority_score['importance_score'].min(axis=0)) / (
        priority_score['importance_score'].max(axis=0) - priority_score['importance_score'].min(axis=0))
    importance_score_scaled.insert(i, z)
    c = (priority_score['urgency_score'][i] - priority_score['urgency_score'].min(axis=0)) / (
        priority_score['urgency_score'].max(axis=0) - priority_score['urgency_score'].min(axis=0))
...
def get_severance(log_r, x_train_count, combined_df_preprocessed, combined_df, df, google_neg_rev_recent, similar_index, APPID, PLAYSTORE_ID):
  ...
  features = pickle.load(
      open(f"dataset/feature_names_{APPID}_{PLAYSTORE_ID}.pkl", "rb"))
  coeffs = log_r.coef_
  result_dict = dict(tuple([(features[i], coeffs[0][i])
                            for i in range(vocab_size)]))
  result_df = pd.DataFrame(
  ...
  lengths = combined_df_preprocessed['review'].apply(
  ...
  combined_df_preprocessed['probabilities'] = combined_df_preprocessed['z_score'].apply(
      lambda x: 1-stats.norm.cdf(x))
  ...
  log_r_scored_df_scaled = outlier_scaling(
      log_r_scored_df, upper_outlier, lower_outlier)

  #get the final dataframe
  combined_df['score'] = log_r_scored_df_scaled['score']
  final_df = pd.merge(combined_df.loc[similar_index][[
                      'review', 'score']], df, how='inner', on='review')
  # fill null with zero
  ...
  sum_dict, normalized_sum_dict = get_summed_scores(
      google_neg_rev_recent, df.columns[1:7].values.astype(str))
  #Sort the value
  sum_dict = {k: v for k, v in sorted(
      sum_dict.items(), key=lambda item: item[1])}
  normalized_sum_dict = {k: v for k, v in sorted(
      normalized_sum_dict.items(), key=lambda item: item[1])}
...
def plot_dict(dict_value):
  ...
  bar = sns.barplot(x=list(dict_value.keys()), y=list(
      dict_value.values()), ax=ax, palette='rocket')
...
def load_dataset(APPID, PLAYSTORE_ID):
  df = pd.read_csv(
      f"dataset/labeled_negative_reviews_with_versions_ratings_type_{APPID}_{PLAYSTORE_ID}.csv")
  combined_df = pd.read_pickle(
      f'dataset/combined_df_{APPID}_{PLAYSTORE_ID}.pkl')
  combined_df_preprocessed = pd.read_pickle(
      f'dataset/data_clean_and_mapped_{APPID}_{PLAYSTORE_ID}.pkl')
  ...
  x_train_count = pickle.load(
      open(f"dataset/x_train_count_{APPID}_{PLAYSTORE_ID}.pkl", "rb"))
  x_test_count = pickle.load(
      open(f"dataset/x_test_count_{APPID}_{PLAYSTORE_ID}.pkl", "rb"))

  x_train_tfidf = pickle.load(
      open(f"dataset/x_train_tfidf_{APPID}_{PLAYSTORE_ID}.pkl", "rb"))
  x_test_tfidf = pickle.load(
      open(f"dataset/x_test_tfidf_{APPID}_{PLAYSTORE_ID}.pkl", "rb"))

  y_train = pickle.load(
      open(f"dataset/y_train_{APPID}_{PLAYSTORE_ID}.pkl", "rb"))
  y_test = pickle.load(
      open(f"dataset/y_test_{APPID}_{PLAYSTORE_ID}.pkl", "rb"))
