...
def google_scrapper(PLAYSTORE_ID, COUNTRY, db_connection):
  google_sqlite_table = f'{PLAYSTORE_ID}_{COUNTRY}'
  ...
  if res.status_code == 200:
    ...
    data = [df_google_ps['reviewId'], df_google_ps['content'],
            df_google_ps['reviewCreatedVersion'], df_google_ps['score'], df_google_ps['at']]
    ...
    df_google = df_google.set_index('reviewId')
    
    datenow = datetime.today().strftime('%d/%m/%Y').replace('/', '')
    filename = f'{PLAYSTORE_ID}_{COUNTRY}_{datenow}_GPS.csv'

    # Disable saving to CSV to save memory 
    # df_google_ps.to_csv(f"dataset/{filename}") 
    ...
    delta_timeunit = 1000
    run_date = datetime.now() + timedelta(seconds=delta_timeunit)
